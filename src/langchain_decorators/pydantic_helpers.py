import inspect
from typing import Dict, Tuple, Type, get_origin, Annotated
import os
    ...
    for field, field_info in field_items:
        ...
        if field in data:
            ...
            if not data_with_compressed_keys:
                data_with_compressed_keys = {
                    k.lower().replace(" ", ""): v for k, v in data.items()
                }
    ...
    if schema.get("definitions"):
        ...
        for def_key, definition in definitions.items():
            ...
            nested_ref = next(
                (
                    1
                    for val in definition.get("properties", {})
            ...
            if nested_ref:
                raise Exception(
                    f"Nested $ref not supported! ... probably recursive schema: {def_key}"
                )
            ...
            if isinstance(schema, dict):
                if schema.get("properties"):
                    for k, v in schema.get("properties").items():
                        if v.get("$ref"):
                            schema["properties"][k] = definitions[
                                v["$ref"].split("/")[-1]
                            ]
