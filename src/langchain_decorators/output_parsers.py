...
class OutputParserExceptionWithOriginal(OutputParserException):
    ...
    def __init__(
        ...
        message: str,
        original: str,
        original_prompt_needed_on_retry: bool = False,
        error_code: int = 0,
    ) -> None:
    ...
    def parse(self, text: str) -> bool:
        ...
        if not match:
            raise OutputParserExceptionWithOriginal(
                message=self.get_format_instructions(),
                original=text,
                original_prompt_needed_on_retry=True,
...
class JsonOutputParser(BaseOutputParser):
    ...
    def replace_json_block(self, text: str, replace_func: Callable[[dict], str]) -> str:
        try:
            ...
            msg = f"Invalid JSON\n {text}\nGot: {e}"
            raise OutputParserExceptionWithOriginal(
                msg, text, error_code=ErrorCodes.INVALID_JSON
            )
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logging.warning(
                        "We might have been able to fix this output using json_repair. You can try json autorepair by installing json_repair package (`pip install json_repair`)"
                    )
                    pass
                raise e
            return json_dict
...
class PydanticOutputParser(BaseOutputParser[T]):
    ...
    def __init__(
        ...
        super().__init__(
            model=model,
            instructions_as_json_example=instructions_as_json_example,
            as_list=as_list,
        )
    ...
    def parse(self, text: str) -> T:
        try:
            ...
            match = re.search(
                regex_pattern, text.strip(), re.MULTILINE | re.IGNORECASE | re.DOTALL
            )
            json_str = ""
            if match:
                json_str = match.group()
            ...
            try:
                ...
                raise OutputParserExceptionWithOriginal(
                    f"Data are not in correct format: {json_str or text}\nErrors: {err_msg}",
                    text,
...
class MarkdownStructureParser(ListOutputParser):
        ...
        if model:
            for field, field_info in model.__fields__.items():
                ...
                if get_field_type(field_info) == list:
                    ...
                    all_sub_str = all(
                        True
                        for sub_field_info in field_type.__fields__.values()
                    ...
                    if all_sub_str:
                        ...
                        self.sections_parsers[field] = MarkdownStructureParser(
                            model=field_type,
                            sections_parsers=sections_parsers.get(field),
                            level=level + 1,
                        )
                    else:
    ...
    def get_instructions_for_sections(
        self,
        model: Type[T] = None,
        sections_parsers: Dict[str, BaseOutputParser] = None,
    ) -> str:
    ...
    def parse(self, text: str) -> List[str]:
        ...
        sections_separators = list(
            re.finditer(r"^#+[ |\t]+(.*)$", text, flags=re.MULTILINE)
        )
        res = {}
...
def _get_str_field_description(
    field_info: ModelField, ignore_nullable: bool = False, default="?"
):
    if USE_PYDANTIC_V1:
