...
if TYPE_CHECKING:
    from .llm_protocols import LlmProtocolBase
...
class LlmSelector(BaseModel):
    ...
    def __init__(
        self,
        generation_min_tokens: int = None,
        prompt_to_generation_ratio: float = 1 / 3,
    ):
        ...
        """
        super().__init__(
            generation_min_tokens=generation_min_tokens,
    ...
    def with_llm_rule(
        ...
        i = len(self.rules)
        self.rules.insert(
            i, dict(max_tokens=max_tokens, llm_selector_rule_key=llm_selector_rule_key)
        )
        self.llms[i] = llm
        return self
    ...
    def get_llm(
        ...
        total_tokens_estimate = self.get_expected_total_tokens(
            prompt,
        ...
        if total_tokens_estimate < first_token_threshold and not llm_selector_rule_key:
            ...
            total_tokens = self.get_expected_total_tokens(
                prompt,
                function_schemas=function_schemas,
                estimate=False,
                expected_generated_tokens=expected_generated_tokens,
            ...
            if llm_selector_rule_key and not key_match:
                raise Exception(
                    f"Could not find a LLM for key {llm_selector_rule_key}. Valid keys are: {set([rule.get('llm_selector_rule_key') for rule in self.rules])}"
                )
            if result_index == None:
                result_index = best_match
        print_log(
            f"LLMSelector: Using {'default' if result_index==0 else str(result_index)+'-th'} LLM: {getattr(self.llms[result_index],'model_name', self.llms[result_index].__class__.__name__)}",
            logging.DEBUG,
        )
        if streaming:
            if result_index not in self.streamable_llms_cache:
                self.streamable_llms_cache[result_index] = make_llm_streamable(
                    self.llms[result_index]
                )
            return self.streamable_llms_cache[result_index]
        else:
            return self.llms[result_index]

    def get_expected_total_tokens(
        self,
        prompt: Union[str, List[BaseMessage]],
        function_schemas: List[dict] = None,
        estimate: bool = True,
        ...
        expected_generated_tokens = (
            expected_generated_tokens or self.generation_min_tokens or 0
        )
        prompt_tokens = self.get_token_count(
            prompt, function_schemas=function_schemas, estimate=estimate
        )
    ...
    def get_token_count(
        ...
        if estimate:
            ...
            num_tokens = count_tokens(
                prompt, llm=self.llms[0]
            )  # note: we will use the first llm to count the tokens... it should be the same general type, and if not, it's ok, should be close enough

        if function_schemas:
            num_tokens += self.get_token_count(
                json.dumps(function_schemas), estimate=estimate
            )
        return num_tokens
...
class GlobalSettings(BaseModel):
    ...
    def get_default_llm(cls):
        return cls.get_current_settings().default_llm
    ...
    def define_settings(
        ...
        **kwargs,
        ...
        if isinstance(default_llm, str):
            default_llm = init_chat_model(default_llm)
        ...
        if verbose is None:
            verbose = os.environ.get("LANGCHAIN_DECORATORS_VERBOSE", False) in [
                True,
                "true",
                "True",
                "1",
...
class PromptTypeSettings:
    ...
    def __init__(
        ...
        llm_protocol: "LlmProtocolBase" = None,
    ...
    @property
    def llm_protocol(self):
        # lazy init due to circular imports
        if not self._llm_protocol:
            from .llm_protocols import LlmProtocolBase

            self._llm_protocol = LlmProtocolBase.get_for_llm(self.llm)
        return self._llm_protocol

    def as_verbose(self):
        return PromptTypeSettings(
            llm=self.llm,
            color=self.color,
            log_level=100,
