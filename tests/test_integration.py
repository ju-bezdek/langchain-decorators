...
class TestIntegrationScenarios:
    ...
    def test_research_assistant_with_memory(self):
        """Test research assistant that maintains conversation memory"""
        ...
        def search_knowledge_base(query: str) -> str:
            ...
            for topic, info in knowledge_base.items():
                if topic in query_lower or any(word in query_lower for word in topic.split()):
                    return f"Found information about {topic}: {info}"
            return f"No specific information found for '{query}' in knowledge base"

        @llm_function
        def save_research_note(topic: str, note: str) -> str:
            ...
            return f"Research note saved for '{topic}' at {timestamp}: {note}"
        ...
        def research_assistant(
            ...
            """
            ```<prompt:system>
            You are a research assistant. Help users find information and maintain context.
            Use the search function to find relevant information.
            Save important insights as research notes.
            ```
        ...
        # First query
        result1 = research_assistant(
            user_query="Tell me about machine learning",
            functions=[search_knowledge_base, save_research_note],
            memory=memory,
        )

        if (
            result1.is_function_call
            and result1.function_name == "search_knowledge_base"
        ):
            ml_info = result1.execute()
            assert "machine learning" in ml_info.lower()

            # Save to memory
            memory.save_context(
                {"input": "Tell me about machine learning"}, {"output": ml_info}
            )

        # Follow-up query with context
        result2 = research_assistant(
            user_query="How does that relate to neural networks?",
            functions=[search_knowledge_base, save_research_note],
            memory=memory,
        )

        # Should be able to handle the contextual reference
        assert isinstance(result2, OutputWithFunctionCall)
